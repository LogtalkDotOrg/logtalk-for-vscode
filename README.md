...
When debugging in the integrated terminal using the `debugger` tool, the current clause (at leashed unification ports) is shown in the active editor window. The VSCode debug toolbar buttons are supported and extended with additional buttons for common port commands (in the left group), top-level solutions backtracking and commit commands(in the middle group), and `debugger` tool messages (in the right group). For other port commands, you must type the command in the integrated terminal.
