...
### Logtalk compiler output parsing

	"logtalk.tail.command": "tail"
	"logtalk.tail.arguments" [ ]

Tail command to capture the Logtalk compiler output for parsing warnings and errors. The arguments **must** be set for the tail command to act in the `$LOGTALKUSER/scratch/.messages` file with the number of lines to output being zero. For example:

	"logtalk.tail.command": "/usr/local/bin/gtail"
	"logtalk.tail.arguments" [ "-f", "/Users/pmoura/logtalk/scratch/.messages", "-n", "0" ]

On Windows systems, these settings must be set differently. For example:

	"logtalk.tail.command": "C:/Program Files/PowerShell/7/pwsh.exe"
	"logtalk.tail.arguments" [ "Get-Content", "C:\\Users\\jdoe\\Documents\\Logtalk\\scratch\\.messages", "-Wait", "-Tail", "0" ]
